...
size_t ctx_modulus_size(context_t* ctx) {
  auto modulus =ctx->ctx->context->first_context_data()->parms().coeff_modulus();
  return modulus.size();
}

uint64_t* ctx_coeffs_modulus(context_t* ctx) {
  auto &modulus = ctx->ctx->context->first_context_data()->parms().coeff_modulus();
  auto ptr = new uint64_t[modulus.size()];
  for (size_t i = 0; i < modulus.size(); i++) {
      ptr[i] = modulus[i].value();
  }
  return ptr;
}
...
uint64_t* a_get_data(a_t* a) {
  return a->a.begin();
}

size_t a_get_size(a_t* a) {
  return a->a.size();
}
