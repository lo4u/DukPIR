...
func (ctx *Context[T]) ModulusSize() uint64 {
	return uint64(C.ctx_modulus_size(ctx.ctx))
}

func (ctx *Context[T]) Modulus() []uint64 {
	ptr := C.ctx_coeffs_modulus(ctx.ctx)
	return unsafe.Slice((*uint64)(ptr), ctx.ModulusSize())
}
...
func (a *A) Size() uint64 {
	return uint64(C.a_get_size((*C.a_t)(a)))
}

func (a *A) GetData() []uint64 {
	ptr := C.a_get_data((*C.a_t)(a))
	size := a.Size()
