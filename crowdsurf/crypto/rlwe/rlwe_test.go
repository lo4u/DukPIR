...
func TestA(t *testing.T) {
	...
	// Sample seeds for A poly
	num := int(math.Ceil(float64(2048) / float64(ctx.N())))
	seeds := make([]uint64, SealSeedLength*num)
	buf := make([]byte, 8)
	for i := range seeds {
		io.ReadFull(rng, buf)
	...
	seed := seeds[0*SealSeedLength : (0+1)*SealSeedLength]
	a := NewA(key, seed)
	defer a.Free()
	t.Log("ctx modulus_size: ", ctx.ModulusSize())
	t.Log("ctx modulus: ", ctx.Modulus())
	t.Log("a size: ", a.Size())
	t.Log("a data len: ", len(a.GetData()))
	// t.Log(a.GetData())
}
